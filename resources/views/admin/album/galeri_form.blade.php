    	...
    	<div class="col-xs-12">
    		<div class="panel panel-default">
    			<div class="panel-heading">
    				Form Galeri Album {{ $album->judul }}
    			</div>
				<form action="{{ @$galeri ? url('admin/album/' . $galeri->id_album . '/galeri/' . $galeri->id_galeri . '/update') : url('admin/album/' . $album->id_album . '/store') }}" method="POST" enctype="multipart/form-data" class="form-galeri">
					{{ csrf_field() }}
					@if (@$galeri)
					{{ method_field('PATCH') }}
					@endif
					<div class="panel-body">
						<div class="form-group">
							<label>Judul</label>
							<input type="text" name="judul" class="form-control" value="{{ old('judul') ? old('judul') : (@$galeri ? $galeri->judul : '') }}">
						</div>
						<div class="form-group">
							<label>Deskripsi</label>
							<textarea class="form-control" name="deskripsi" id="editor" rows="5">{{ old('deskripsi') ? old('deskripsi') : (@$galeri ? $galeri->deskripsi : '') }}</textarea>
						</div>
						<div class="form-group">
					...
					<div class="panel-footer text-right">
						<a class="btn" href="{{ url('admin/galeri') }}">Cancel</a>
